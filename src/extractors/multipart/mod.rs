...
pub type AppHttpResponse = Result<actix_web::HttpResponse, actix_web::Error>;
...
pub async fn store_file(upload_path: &str, identifier: &str, path_prefix: &str, 
    asset: std::sync::Arc<tokio::sync::Mutex<Multipart>>) -> Result<String, AppHttpResponse>{
    ...
    /*  
        ...
        field object for writing the bytes into the file, each field 
        represnets a future object
    */
    while let Ok(Some(mut field)) = asset.try_next().await{
        ...
        */
        ...
        /* 
            ...
            object to extract the bytes, we can use tokio file and writing into
            it inside tokio spawn but we need to use Arc<Mutex<File>>
        */
        let cloned_filepath = filepath.clone();
        let mut f = web::block(|| std::fs::File::create(cloned_filepath).unwrap()).await.unwrap();
        ...
        // let tokiofile = tokio::fs::File::create(filepath).await.unwrap();
        // let arced_mutexed_tokiofile = std::sync::Arc::new(tokio::sync::Mutex::new(tokiofile));
        
        // tokio::spawn(async move{

        //     let mut file_lock = arced_mutexed_tokiofile.lock().await;
        //     (*file_lock).write_all(&file_buffer).await;
...
pub async fn extract(
...
) -> Result<(serde_json::Value, HashMap<String, Vec<u8>>), AppHttpResponse>{
    ...
    /* ------------------------------------------------------------------------
        ...
        /* 
            json_data can by any content, here we're trying to map it into the AnyJsonDataStruct struct 
            note that AnyJsonDataStruct is a json string and all its fields are of type String cause we 
            didn't know the exact type of each field while we were extracting we assume all them are type 
            of String later we can parse each field to its related type.
        */
        let json_data_instance = serde_json::from_value::<AnyJsonDataStruct>(json_data).unwrap();
